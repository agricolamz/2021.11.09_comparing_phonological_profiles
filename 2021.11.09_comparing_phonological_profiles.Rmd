...
* Parametric typology:
    ...
    * apply Hamming distance,\footnote[frame]{That is just ordered version of Jaccard distance: ratio of similar units in two languages out of all possible units in two languages.}
...
It is worth noting all arguments against using segment inventories as a measure [@simpson99; @ohala09; @coupe09; @deutscher09], but dictionaries and corpora are not freely available for any language.
...
* the distance-based approach is better in capturing all phonotactic details present in the data (as [@maddieson09] proposed), but it depends on more or less comparable data from different languages.

It is also worth mentioning that both [@heeringa04] and [@eden18] discuss and apply different methods for acoustic distances across languages, but I decided to categorize it as a different topic.
